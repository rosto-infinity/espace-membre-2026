...
Un routeur **mappe les requêtes HTTP à des fonctions de gestion** en fonction des patterns d'URL et des méthodes HTTP.
...
class Router {
    ...
    private function matchPath(string $routePath, string $uri): array|false {
        // Convertir /users/{id} en regex
        $pattern = preg_replace('/\{([a-z]+)\}/', '(?P<$1>[^/]+)', $routePath);
        $pattern = '#^' . $pattern . '$#';

        if (preg_match($pattern, $uri, $matches)) {
            // Retourner seulement les captures nommées
